...
const Result& BallisticTable::at(int iz, int iv, int im, int id, int il) const
{
...
Interp BallisticTable::findInterp(float val, const std::vector<float>& axis) const
{
  if (val <= axis.front())
    return {0, 0.0f};
  if (val >= axis.back())
...
Result BallisticTable::lookup(float Z0, float V0, float m, float d, float l) const
{
